// trakt-media-list-filter: a list creater for trakt
package main
...
func (cm *configManager) getConfig(key string) string { return cm.config[key] }
...
func exchangeCode(cm *configManager, code string) string {
	...
	_, err := client.R().
		SetFormData(map[string]string{
			"code": code, "client_id": cm.getConfig("CLIENT_ID"),
...
func run(cmd *cobra.Command, args []string) {
	...
	filtered := filterByRole(items, role, moviesOnly, tvOnly, all)
	if listName == "" {
...
func addMediaToList(client *resty.Client, listID int, items []filtered) {
	...
	for i := 0; i < len(items); i += batch {
		...
		var movies, shows []map[string]interface{}
		for _, it := range items[i:end] {
			if it.Type == "movie" {
				movies = append(movies, map[string]interface{}{"ids": map[string]int{"trakt": it.ID}})
			} else {
				shows = append(shows, map[string]interface{}{"ids": map[string]int{"trakt": it.ID}})
			}
		}
		if len(movies) > 0 {
			_, _ = client.R().
				SetBody(map[string]interface{}{"movies": movies}).
				Post(fmt.Sprintf("https://api.trakt.tv/users/me/lists/%d/items", listID))
		}
		if len(shows) > 0 {
			_, _ = client.R().
				SetBody(map[string]interface{}{"shows": shows}).
				Post(fmt.Sprintf("https://api.trakt.tv/users/me/lists/%d/items", listID))
		}
		time.Sleep(time.Second)
	}
